...
/*   Updated: 2023/04/18 00:44:50 by agengemb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	else if (!cmd->cmd[2])
	{
		if (ft_strlen(cmd->cmd[1]) == 1 && cmd->cmd[1][0] == '-')
		{
			...
			if (old_pwd)
				...
				return (error("OLDPWD is not present\n"), NULL);
		}
		return (replace_home(cmd, default_folder));
	}
	mess_error = "too many arguments\n";
	write(p->default_out, "cd: ", 4);
	write(p->default_out, mess_error, ft_strlen(mess_error));
	return (free(default_folder), NULL);
}
